...
public class AlarmNotification extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        System.out.println("reminder dbRowId: " + tableRowId);
        ...
        builder.setAutoCancel(true)
                ...
                .setContentIntent(pNotificationIntent);// 알림 클릭 시 설정한 Activity로 이동
        ...
        if (notificationManager != null) {
            ...
            System.out.println("reminder " + cursor.getString(ToDoTable.FIELD_NAME_ALARM));

            LocalDateTime now = LocalDateTime.now();
            String formatNow = now.format(DateTimeFormatter.ofPattern("yyyy/MM/dd/HH/mm")); //현재날짜 설정
            System.out.println("reminder " + formatNow);

            //알림 취소되었거나 알림이 수정되었다면
            if (cursor.getString(ToDoTable.FIELD_NAME_ALARM).equals("0000/00/00/00/00") || !cursor.getString(ToDoTable.FIELD_NAME_ALARM).equals(formatNow)) {
